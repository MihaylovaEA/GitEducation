...
Решить конфликкт можно двумя способами:
...
git branch -d local_branch_name - команда, выполняющая удаление ветки. Вызывать данную команду необходимо находясь в другой ветке.

## Игнорируемые файлы

В большинстве проектов есть файлы или целые директории, которые мы не хотим (и, скорее всего, не захотим) коммитить. Мы можем удостовериться, что они случайно не попадут в git add -A при помощи файла .gitignore. Для этого необходимо выполнить ряд действий:

1. Создать вручную файл под названием .gitignore и сохраните его в директорию проекта.
